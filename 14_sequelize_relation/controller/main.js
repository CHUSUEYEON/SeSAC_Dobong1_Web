// const models = require("../models"); 이렇게 쓰던가
// models = {Player, Profile, ...Player, sequelize, Sequelize}

const { Op } = require("sequelize");
const { Player, Profile } = require("../models"); //구조분해할당 이용해서 이렇게 쓰던가 둘 중 택 1
...
exports.getAllPlayer = async (req, res) => {
  try {
    const players = await Player.findAll(); //위에서 구조분해할당 안 했을 경우 이렇게 models.Player.findAll()
    console.log(players);
    res.send(players);
  } catch (err) {
    console.log("err", err);
