...
// ver2. 암호화된 쿠키
app.use(cookieParser("secretKey")); //문자열 내 마음대로 써도 됨.

const cookieConfig = {
  /*
    - maxAge : 쿠키의 수명, 1000이 1초인 밀리초 단위
    - expires : 만료 날짜, GMT 시간 설정
    - httpOnly : http 통신망 접근 허용(true /false)
...
- 쿠키 설정 >> res.cookie(이름, 값, 옶션)
- 쿠키 확인 >> req.cookies 객체에서 확인
- 쿠키 삭제 >> req.clearCookie(이름, 값, 옵션)
    - 쿠키 삭제만 하는 것이고 응답 완료까진 하지 않음(end or send 처리 해줘야 함)
    - 이름, 값, 옵션이 정확이 일치해야 삭제됨
        (expires, masAge 옵션은 일치하지 않아도 됨.)
