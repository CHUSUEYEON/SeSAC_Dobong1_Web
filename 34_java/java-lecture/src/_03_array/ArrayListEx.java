...
//        List<E> numbers = new ArrayList<>(); // E에 지정된 타입의 객체만 저장 가능
//        List numbers = new ArrayList<>(); // 모든 타입의 객체만 저장 가능
        // 1. arrayList 생성하기

        List<Integer> numbers = new ArrayList<>();
        System.out.println(numbers); //[] Arrays.toString 사용하지 않아도 됨
...
//        numbers.add(인덱스, 삽입할 값);
        numbers.add(1, 7);
        System.out.println("요소 삽입 후: " + numbers); //[10, 7, 77, 30, 40, 50]

        // 6. ArrayList 끼리 연결: arrList1.addAll(arrList2)
        ...
        System.out.println(numbers.indexOf(11)); // 요소가 없을 때, -1 (무조건 -1, 무작위 아님)

        // 8. 요소 삭제 .remove(index)
        System.out.println("삭제 전" + numbers);  // [10, 7, 77, 30, 40, 50, 100, 99, 98]
        numbers.remove(2); // 2번 인텍스의 요소 삭제
        System.out.println("삭제 후" + numbers); //[10, 7, 30, 40, 50, 100, 99, 98]

        // 9. 리스트의 크기 확인
//        System.out.println(numbers.length); // length는 사용하면 안돼요.
